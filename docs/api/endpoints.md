...
живут до истечения срока). Для аккаунтов, зарегистрированных по паролю, ключ при
регистрации не выпускается — первый вызов этого эндпоинта выполняет **первичную
выдачу**. Ответ 200:
...
1. **Заголовок `x-api-key`.** В коде существует legacy-механизм статического
...
2. **`POST /clients/{id}/upgrade` без оплаты.** Смена тарифа сейчас работает без
...
3. **Горизонты тарифов.** Код (`PLAN_SPECS`) сейчас применяет Free 7 /
   Start 30 / Business 90 дней, и документация описывает это фактическое
   поведение. Решением владельца (2026-07-19) значения 30/90 признаны
   устаревшими относительно пересмотренной бизнес-модели (горизонт 1–28);
   финальные капы по тарифам и синхронная правка кода и документации —
   corefundev/backend#542.
4. **`datasets_limit` (1/3/10).** В коде значения помечены как плейсхолдеры до
   решения владельца по тарифной сетке — подтвердить перед широкой публикацией.
5. **Политика версионирования.** Путей с версионным префиксом нет; формальная
