...
def _rec(client_id: str = "acme", plan: str = "business") -> ClientRecord:
    # business plan = no cooldown, so only the in-flight clause of the
    # gate is under test.
...
def test_first_claim_succeeds_and_sets_status_training(tmp_path):
    r = _reg(tmp_path)
    r.register(_rec())
    res = r.try_record_training_run("acme", None, _now())
...
def test_second_concurrent_claim_is_blocked(tmp_path):
    r = _reg(tmp_path)
    r.register(_rec())
    assert r.try_record_training_run("acme", None, _now()) is not None
    # already in flight → second claim denied (the clobber window)
    assert r.try_record_training_run("acme", None, _now()) is None
