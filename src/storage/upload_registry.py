...
# DP-4b (#324): prep columns persisted as JSONB — settable via update_fields (no
# status change) and, for report/proposal, alongside the NEEDS_MAPPING transition.
...
class PostgresUploadRegistry(UploadRegistry):
    ...
    def update_fields(self, upload_id: str, **fields) -> UploadRecord:
        ...
        Used to attach the sniff report / mapping proposal / confirmed mapping
        while the upload stays at SCANNED_CLEAN or NEEDS_MAPPING (a self-status
